...
function autoCalcSteps(orders) {
   ...
   let nodeValue = getDistanceTotal(curr);
   opens.push({
      v: nodeValue,
      step: 0,
      order: curr
   ...
   while (opens.length !== 0) {
      ...
      //判断当前节点的可能产生的节点
      let currentNode = opens.splice(minOfArray(opens), 1)[0];
      closeMap.set(currentNode.order, currentNode);

      //判断是否到达最终状态
      if(currentNode.order == '012345678') {
         console.log('寻路成功！', currentNode);
         // console.log(close);
         let paths = getPath(currentNode)
         let pathNames = [];
...
function minOfArray(arr) {
   let index = 0, length = arr.length, min = arr[0];
...
function isChange(current, next) {
   //对比两个数组
   let currArr = current.split('').map(Number);
   let nextArr = next.split('').map(Number);
...
//扩展目标函数的子节点
function getNextOrders(order) {
   let curr = order.split('').map(Number);
...
//状态距离还原状态位置 
function getDistanceTotal(order) {
   let orders = order.split('').map(Number)
   let total = 0;
